...
CREDENTIALS_FILE = os.environ.get(
    ...
    os.path.join("credentials", "google_drive_credentials.json")
)
...
async def test_drive_connector():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                files = await connector.list_files(max_results=10)
            ...
            try:
                folder = await connector.create_folder(folder_name)
                ...
                file_content = await connector.download_file(file_id)
                ...
                # Test deleting the file (optional)
                if input("\nDelete test file and folder? (y/n): ").lower() == 'y':
                    logger.info("Deleting test file...")
                    delete_result = await connector.delete_file(file_id)
                    logger.info(f"File deletion result: {delete_result}")
                    
                    logger.info("Deleting test folder...")
                    delete_result = await connector.delete_file(folder_id)
                    logger.info(f"Folder deletion result: {delete_result}")
